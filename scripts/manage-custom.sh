...
# Claude Relay Service 自定义管理脚本（mcrs）
# 用于安装、更新、卸载、启动、停止、重启服务（支持自定义分支）
# 可以使用 mcrs 快捷命令调用
...
persist_install_path() {
    local conf_dir="$HOME/.config/mcrs"
...
install_service() {
    ...
    if ! git clone https://github.com/whoismonay/claude-relay-service.git "$APP_DIR"; then
        print_error "克隆项目失败"
        return 1
    fi
...
EOF
    ...
    if git ls-remote --heads origin web-dist | grep -q web-dist; then
        ...
        git clone --depth 1 --branch web-dist --single-branch \
            https://github.com/whoismonay/claude-relay-service.git \
    ...
    echo -e "\n${YELLOW}管理命令：${NC}"
    echo "  查看状态: mcrs status"
    echo "  停止服务: mcrs stop"
    echo "  重启服务: mcrs restart"
}
...
update_service() {
    ...
    # 检查前端文件（已通过 git pull 更新）
    print_info "检查前端文件..."

    # 前端文件应该已经通过 git pull 更新，直接检查是否存在
    if [ -f "web/admin-spa/dist/index.html" ]; then
        print_success "前端文件已更新"
    else
        print_warning "前端文件不存在，尝试本地构建..."

        # 回退方案：本地构建
        if command_exists npm && [ -f "web/admin-spa/package.json" ]; then
            print_info "开始本地构建前端..."
            mkdir -p web/admin-spa/dist
            cd web/admin-spa
            npm install
            ...
            print_error "无法构建前端文件，请检查 Node.js 环境"
            return 1
        fi
    fi
...
switch_branch() {
    ...
    echo ""
    print_info "提示：如遇到问题，可以运行 'mcrs update' 强制更新到最新版本"
}
...
# 显示帮助
show_help() {
    echo "Claude Relay Service 自定义管理脚本（mcrs）"
    echo ""
    echo "用法: mcrs [命令]"
    echo ""
    echo "说明: 此脚本支持更新当前分支（myMain 或功能分支），而非固定 main 分支"
    echo ""
    echo "命令:"
    echo "  install        - 安装服务（克隆自定义 fork 仓库）"
    echo "  update         - 更新服务（更新当前分支）"
    echo "  uninstall      - 卸载服务"
    echo "  start          - 启动服务"
    echo "  stop           - 停止服务"
    echo "  restart        - 重启服务"
...
create_symlink() {
    ...
    local symlink_path="/usr/bin/mcrs"
    ...
    if sudo ln -s "$script_path" "$symlink_path"; then
        print_success "已创建快捷命令 'mcrs'"
        echo "您现在可以在任何地方使用 'mcrs' 命令管理服务"
...
load_config() {
    ...
    if [ -n "$APP_DIR" ] && [ -f "$APP_DIR/package.json" ]; then
        ...
        # 3) 尝试从持久化配置读取安装位置
        if [ -z "$APP_DIR" ]; then
            local conf_file="$HOME/.config/mcrs/install.conf"
            if [ -f "$conf_file" ]; then
